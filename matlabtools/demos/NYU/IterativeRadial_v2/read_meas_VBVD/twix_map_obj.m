...
%
    ...
    methods
        ...
        function this = readMDH(this,mdh,filePos)
            ...
            cAcq      = this.NAcq   + mdh.ulDMALines;  %SB, we want to read all the lines in the DMA
            this.NAcq = cAcq;  
            ...
            for j = 1:mdh.ulDMALines %SB, loop over the lines in the DMA
                cAcqt = cAcq - mdh.ulDMALines + j;
                % save mdh information about current line
                this.NCol       (cAcqt) = mdh.ushSamplesInScan + 0; % +0 significantly faster??!
                this.NCha       (cAcqt) = mdh.ushUsedChannels  + 0;
                this.Lin        (cAcqt) = cLin + (j-1); % SB
                this.Par        (cAcqt) = cPar;
                this.Sli        (cAcqt) = cSli;
                this.Ave        (cAcqt) = cAve;
                this.Phs        (cAcqt) = cPhs;
                ...
                this.Idd        (cAcqt) = cIdd;
                this.Ide        (cAcqt) = cIde;
                this.centerCol  (cAcqt) = mdh.ushKSpaceCentreColumn + 1;
                this.centerLin  (cAcqt) = mdh.ushKSpaceCentreLineNo + 1;
                this.centerPar  (cAcqt) = mdh.ushKSpaceCentrePartitionNo + 1;
                this.IsReflected(cAcqt) = logical(min(bitand(mdh.aulEvalInfoMask(1),2^24),1));
                if ( rem(j+1,2) ) %SB, each next line is reflected versus the previous one
                    this.IsReflected(cAcqt) = not(logical(min(bitand(mdh.aulEvalInfoMask(1),2^24),1))); %SRY
                end;
                this.IsRawDataCorrect(cAcqt) = logical(min(bitand(mdh.aulEvalInfoMask(1),2^10),1)); %SRY
                this.slicePos (:,cAcqt) = mdh.SlicePos + 0;
                this.iceParam (:,cAcqt) = mdh.aushIceProgramPara + 0;
                this.freeParam(:,cAcqt) = mdh.aushFreePara + 0;
                % save memory position
                this.memPos     (cAcqt) = filePos + (j-1)*(mdh.ulDMALength);%(mdh.ulDMALength-this.freadInfo.szScanHeader); %SB, the crucial part, point to the right location in the DMA
                        % subsref adds szScanHeader, so we have to foresee a szScanHeader offset for every position in the
                        % file (in short, we don't have to do anything, since it is already included)
                        
                        % -this.freadInfo.szScanHeader
            end;
        ...
        function varargout = subsref(this, S)
            ...
            bMemMap = false; % scheint viel langsamer zu sein...
            if bMemMap
                % create format for memory mapping:
                format = [
                             {'uint8'},{szScanHeader},{'scanheader'}; % we just skip this
                             {'single'},{double(readSize)},{'data'}   % here's our data
                                                                      % (plus channelHeader info)
                         ]; 

                % sadly, we can't use 'repeat' since the distance may not be
                % constant (e.g. in case other scans (pc,ref,..) are acquired
                % in between)
                m = memmapfile(this.filename,'Format',format,'Offset',mem(1),'Repeat',1);
            else
