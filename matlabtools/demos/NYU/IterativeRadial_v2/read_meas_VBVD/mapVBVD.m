...
%%%%% Parse varargin %%%%%
    ...
    twix_obj.image      = twix_map_obj(arg,'image',filename,version);
    twix_obj.imageM      = twix_map_obj(arg,'imageM',filename,version); %SB
    twix_obj.noise      = twix_map_obj(arg,'noise',filename,version);
    twix_obj.phasecor   = twix_map_obj(arg,'phasecor',filename,version);
    twix_obj.refscan    = twix_map_obj(arg,'refscan',filename,version);
    twix_obj.refscanPC  = twix_map_obj(arg,'refscan_phasecor',filename,version);
    twix_obj.RTfeedback = twix_map_obj(arg,'rtfeedback',filename,version);    
    twix_obj.phasestab  = twix_map_obj(arg,'phasestab',filename,version);
    ...
    while cPos+128<(fileSize-96) % fail-safe; in case we miss MDH_ACQEND %SB
        % display([' ' num2str(cPos + 128) '/' num2str(fileSize) ' ']); %SB
        switch version
            case 'vb'
        ...
        if (mask.MDH_IMASCAN && arg.bReadImaScan && mdh.ulDMALines > 1) % SB
            twix_obj.imageM.readMDH(mdh,cPos);
        end
    ...
    if twix_obj.imageM.NAcq == 0 %SB
        twix_obj = rmfield(twix_obj,'imageM');
    else
        twix_obj.imageM.clean();
    end
...
%     mdh.ushCoilSelect              = dummy(4);
    ...
    % SB
    if (mdh.ulDMALength > (2*4*mdh.ushSamplesInScan + szMDH) * mdh.ushUsedChannels )
        mdh.ulDMALines = max(1,floor(mdh.ulDMALength/((2*4*mdh.ushSamplesInScan + szMDH) * mdh.ushUsedChannels)));
    end;

    % pehses: it seems that the pack bit means that multiple ADC are packed
    % into one DMA, e.g. the echoes from an EPI scan; since this code
    % assumes one adc (x NCha) per DMA, we have to correct the "DMA length"
    if mdh.ulPackBit || (mdh.ulDMALines > 1) %SB
...
function [mdh mask cur_read] = evalMDHvd(fid,cPos)
    ...
    mdh.ushSamplesInScan           = dummy(1);
    mdh.ushUsedChannels            = dummy(2);
%     display([' DMALength ' num2str(mdh.ulDMALength) ' / PackBit ' num2str(mdh.ulPackBit) ' / PCI_rx ' num2str(mdh.ulPCI_rx) ' / SamplesInScan ' num2str(mdh.ushSamplesInScan)]); %SB
    mdh.sLC                        = fread(fid, [1 14], 'ushort');
...
%     mdh.ushCoilSelect              = dummy(4);
    ...
    % SB
    if ( (mdh.ushSamplesInScan > 0) && (mdh.ulDMALength > (szScanHeader + (2*4*mdh.ushSamplesInScan + szChannelHeader) * mdh.ushUsedChannels) ) )
        mdh.ulDMALines = max(1,floor(mdh.ulDMALength/(szScanHeader + (2*4*mdh.ushSamplesInScan + szChannelHeader) * mdh.ushUsedChannels)));
    end;

    % pehses: it seems that the pack bit means that multiple ADC are packed
    % into one DMA, e.g. the echoes from an EPI scan; since this code
    % assumes one adc (x NCha) per DMA, we have to correct the "DMA length"
    if mdh.ulPackBit || ( (mdh.ulDMALines > 1) && (mdh.ushSamplesInScan > 0) ) %SB
