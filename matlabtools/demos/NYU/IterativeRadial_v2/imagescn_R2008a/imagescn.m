function imagescn(I, scale, dims, FigureWidth, timedimension)
warning off
...
% 	end
	if isempty(scale)
		if Nd>4
            imagesc(I(:,:,i0,j0)); axis square; axis off;
		else 		
            imagesc(I(:,:,k)); axis square; axis off;
		end
	elseif size(scale,1)==1
		if Nd>4
            imagesc(I(:,:,i0,j0),scale); axis square; axis off;
        else	
            imagesc(I(:,:,k),scale); axis square;axis off,
            imagesc(I(:,:,k),scale); axis square;axis off
		end
	elseif size(scale,1)==min(N,N1*N2);
		if Nd>4
            imagesc(I(:,:,i0,j0),scale(k,:)); axis square; axis off;
		else		
            imagesc(I(:,:,k),scale(k,:)); axis square;axis off
		end
	end
    if moviemodeflag==1        
        if Nd==3
