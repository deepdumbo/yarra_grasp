...
% First run the optimizer for 1 iterations without penalty terms
% [adjustment]
...
options.MaxIters=param.iterationsPhantom-1;
...
% images with 4x bilinear interpolation (looks better) [adjusted]
figure('Name', 'Gridding FFT'), imagescn(imresize(fliplr(log(abs(fftshift(fft2(fftshift(gridding)))))),   [size(gridding,1)*4 size(gridding,2)*4],      'bilinear'),[],[],[],3);
figure('Name', 'Iterative FFT'),imagescn(imresize(fliplr(log(abs(fftshift(fft2(fftshift(iterative)))))),   [size(gridding,1)*4 size(gridding,2)*4],      'bilinear'),[],[],[],3);
